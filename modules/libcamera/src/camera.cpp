...
int camera::Camera::prepareCamera(int videoid) {
	int ret;
...
int camera::Camera::prepareCameraWithBase(int videoid, int videobase) {

	camerabase = videobase;

	return prepareCamera(videoid);
}

void camera::Camera::processCamera() {
	readframeonce();
}

void camera::Camera::stopCamera() {
	stopcapturing ();
